...
public class Copy_3_of_UpdateHotelServlet extends HttpServlet {
	/**
		...
		if (user==null){
			...
			try {
				...
				{response.sendRedirect("/updatehotelhome?error=HotelManagerMismatch");}
					...
					String pricemin = Filternulls(hotel, "pricemin");
					String pricemax = Filternulls(hotel, "pricemax");
					...
					pw.println("<div id='bigcontainer'>" +
							...
							"				<td class=languagebutton><a href='/updatehotelhomej'>ì˙ñ{åÍ</a></td>" +
							"			</table>" +
							"		</div>" +
							"	<div id=searchmenu>"  +
					...
					pw.println(	"						</select>" +
							...
							"		</form>	" + 
							"		<form action=/goupdatehotel method='post'>" +
							"			<input type='hidden' language='Japanese' />" +
							"			<dt>ãMï˚ÇÃé{ê›ÇëIÇÒÇ≈â∫Ç≥Ç¢ÅB" +
							"					<dt><select name ='hotelKeyString' size=3 required style='width:190px;' >" +
							"						<option value='Choose' disabled selected>Choose . . .</option>");
					for (int j=0; j<hotelKeys.size(); j++){
						if (managerLists.get(j).contains(userID))
					...
					pw.println("				>" +
							...
							"							<input id=welkid type='checkbox' name='welcoming' value='largegroups'" + welgroups + ">large groups" +
					...
					pw.println(	"</textarea>" +
							...
							"					<dd><input type='hidden' name='services' value=''> " +
							"						<input id='touristassociation' type='checkbox' name='type' value='Tourist Association' " + touristassociation + ">Tourist Association<br>" +
